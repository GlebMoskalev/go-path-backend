...
description: "Встраивание структур, отличие от наследования, конфликт имён"
order: 3
---
...
// Но поля и методы доступны напрямую через встраивание
```
...
// Dog имеет метод Describe() через встраивание — значит, удовлетворяет интерфейсу
...
Внешняя структура может «перекрыть» метод встроенной:
...
car.Start()  // метод доступен напрямую, даже через указатель
