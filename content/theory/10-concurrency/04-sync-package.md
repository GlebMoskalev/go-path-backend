...
### wg.Go — новый способ в Go 1.25

В Go 1.25 добавили метод `wg.Go`, который объединяет `Add(1)` + запуск горутины в одну строку:

```go
var wg sync.WaitGroup

for i := 0; i < 5; i++ {
    wg.Go(func() {         // сам вызывает Add(1) и запускает горутину
        fmt.Println(i)
    })
}
...
Это устраняет классическую ошибку — забыть вызвать `Add(1)` или вызвать его внутри горутины. Старый способ через `Add` + `defer Done` по-прежнему работает и нужен когда хочешь добавить сразу несколько единиц счётчика (`wg.Add(n)`).
...
`Once` запоминает что функция была вызвана, даже если та запаниковала. Второй вызов `Do` уже ничего не запустит — с точки зрения `Once` попытка уже была:

```go
var once sync.Once

once.Do(func() {
    panic("ошибка инициализации")  // паника! функция не завершилась нормально
})
// паника поймана recover'ом где-то выше...

once.Do(func() {
    fmt.Println("инициализация")  // не выполнится — Once уже считает себя отработанным
})
```

Это значит что если первый вызов завершился паникой, объект останется неинициализированным, но повторить попытку через тот же `Once` уже невозможно. На практике — паника внутри `Do` это почти всегда баг, который нужно исправлять, а не ловить.
