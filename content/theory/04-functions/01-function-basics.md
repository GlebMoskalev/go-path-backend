...
Функции в Go — первоклассные значения: их можно присваивать переменным, передавать как аргументы и возвращать из других функций. Но начнём с основ.
...
**Ловушка 1**: shadowing именованного возвращаемого значения.

```go
func getUser(id int) (user *User, err error) {
    // ОШИБКА: := создаёт новую переменную user, перекрывая именованную!
    if user, err := db.Find(id); err != nil {
        return nil, err  // возвращаем правильно, но...
    }
    // здесь user из именованного возвращаемого — nil!
    return  // вернёт nil вместо найденного пользователя
}

// Правильно:
func getUser(id int) (user *User, err error) {
    user, err = db.Find(id)  // присваиваем именованным переменным
    return
}
```
