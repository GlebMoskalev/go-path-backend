...
description: "...T синтаксис, передача слайса в variadic, append как пример"
order: 2
---

# Вариадические функции

Вариадические функции принимают переменное количество аргументов одного типа. Самый знакомый пример — `fmt.Println`, которому можно передать сколько угодно значений.
...
**Внутри функции** вариадический параметр `nums` — это обычный `[]int`. Все операции со слайсами применимы:
...
**Важно**: при передаче слайса через `...` данные **не копируются**. Функция получает тот же underlying array:

```go
func appendZero(nums ...int) []int {
    return append(nums, 0)
}

original := []int{1, 2, 3}
result := appendZero(original...)
fmt.Println(original)  // [1 2 3] — зависит от capacity!
fmt.Println(result)    // [1 2 3 0]
```
...
## Вариадические функции с интерфейсом any
