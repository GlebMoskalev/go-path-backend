...
Это самый распространённый случай, где нужен импорт с `_`. Разберём что происходит изнутри.

Пакет `database/sql` из стандартной библиотеки не знает ни о каких конкретных базах данных. Он предоставляет общий интерфейс, а драйверы (PostgreSQL, MySQL, SQLite) регистрируются в нём через функцию `sql.Register`. Ключевое — драйвер регистрирует сам себя в своём `init()`:

```go
// Внутри пакета github.com/lib/pq (это не твой код, это код библиотеки):
package pq

import "database/sql"

func init() {
    // При загрузке пакета — драйвер регистрируется под именем "postgres"
    sql.Register("postgres", &Driver{})
}
```

Теперь в своём коде ты хочешь использовать PostgreSQL. Если просто написать `sql.Open("postgres", ...)` — Go не знает что такое `"postgres"`, драйвер не зарегистрирован, будет ошибка.

Нужно чтобы `init()` из пакета `pq` выполнился. Для этого его достаточно импортировать. Но если ты нигде в коде не используешь имена из этого пакета, компилятор выдаст ошибку «импортирован но не используется». Именно для этого случая и существует `_`:

```go
import (
    "database/sql"
    _ "github.com/lib/pq"  // имена из pq не нужны, но init() должен выполниться
)

func main() {
    // Теперь "postgres" зарегистрирован — работает!
    db, err := sql.Open("postgres", "postgresql://localhost/mydb")
    // ...
}
```

Подчёркивание говорит компилятору: «я знаю что не использую этот пакет напрямую — это сделано специально». Пакет загружается, его `init()` выполняется, драйвер регистрируется.

Этот паттерн встречается везде где есть «реестр с плагинами»: драйверы БД, кодеки изображений (`image/png`, `image/jpeg`), форматы логов.
