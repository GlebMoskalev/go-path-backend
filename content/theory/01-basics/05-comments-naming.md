...
Каждый пакет должен иметь комментарий перед строкой `package`:
...
Запускай `go vet` перед коммитом — это быстро и часто ловит реальные баги.
...
**Важно**: PR с неформатированным кодом в Go-проектах не принимаются. `gofmt` — стандарт, не рекомендация.
