...
Range по каналу читает значения по мере поступления и завершается только после `close(ch)` — без закрытия цикл заблокируется навсегда.
...
// В switch break выходит из switch, а не из внешнего цикла:
for i := 0; i < 5; i++ {
    switch i {
    case 3:
        break  // выходит из switch, цикл продолжается
    }
    fmt.Println(i)  // напечатает 0 1 2 3 4
}
```
...
**Ошибка 2**: Рассчитывать на порядок итерации по слайсу, полученному из неупорядоченных данных — или по map.

```go
// Порядок range по map непредсказуем при каждом запуске:
for k, v := range scores {
    fmt.Println(k, v)  // порядок каждый раз разный
}
// Если нужен фиксированный порядок — собирай ключи в слайс и сортируй.
```

> 💡 Ошибки с каналами (незакрытый канал и зависший range) разберём в главе 10 «Конкурентность».
