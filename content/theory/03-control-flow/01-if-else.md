...
// ...но не здесь! file видна только внутри if/else
```
...
**Ошибка 1**: Забыть, что переменная из init statement не видна снаружи.
...
**Ошибка 3**: Забыть проверить второе возвращаемое значение.

```go
val := m["key"]  // val == "" если ключа нет, но мы не знаем, есть ли ключ

val, ok := m["key"]  // правильно: проверяем наличие ключа
if !ok {
    // ключ не найден
}
```
