...
`switch` в Go отличается от C и Java: каждый `case` завершается автоматически, без `break`. Несколько значений можно перечислить в одном `case`, `switch` можно писать без выражения (как замену цепочке `if/else`), а отдельный вариант — type switch — позволяет ветвиться по типу значения.
