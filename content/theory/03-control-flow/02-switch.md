...
**Ключевое отличие от C/Java**: в Go `break` в конце каждого case **по умолчанию подразумевается**. Выполнив один case, `switch` завершается — не проваливается в следующий:
...
**Важно**: `fallthrough` выполняет следующий case **безусловно**, не проверяя его значение. Используется редко — только когда действительно нужна такая логика.
...
- `switch` в Go не проваливается в следующий case — `break` не нужен
- Несколько значений в одном case: `case "a", "b", "c":`
- `switch {}` без выражения = удобная замена длинному `if/else if`
- `fallthrough` — явный провал в следующий case, используется редко
